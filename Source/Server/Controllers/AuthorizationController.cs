...
using LegendaryTelegram.Server.ViewModels.Authorization;
...
public class AuthorizationController : Controller
{
        ...
        switch (await this.applicationManager.GetConsentTypeAsync(application))
        {
            ...
            case ConsentTypes.Explicit when authorizations.Any() && !request.HasPrompt(Prompts.Consent):
                ...
                var identity = new ClaimsIdentity(OpenIddictServerAspNetCoreDefaults.AuthenticationScheme)
                    .AddClaim(Claims.Subject, await this.userManager.GetUserIdAsync(user))
                    .AddClaim(Claims.Email, await this.userManager.GetEmailAsync(user))
                    .AddClaim(Claims.Name, await this.userManager.GetUserNameAsync(user))
                    .AddClaims(Claims.Role, (await this.userManager.GetRolesAsync(user)).ToImmutableArray());

                // Note: in this sample, the granted scopes match the requested scope
                // but you may want to allow the user to uncheck specific scopes.
                // For that, simply restrict the list of scopes before calling SetScopes.
                identity.SetScopes(request.GetScopes());
                identity.SetResources(await this.scopeManager.ListResourcesAsync(identity.GetScopes()).ToListAsync());
                ...
                identity.SetAuthorizationId(await this.authorizationManager.GetIdAsync(authorization));
    ...
    public async Task<IActionResult> Exchange()
    {
            ...
            || request.IsRefreshTokenGrantType())
        {
            ...
            var user = await this.userManager.FindByIdAsync(principal.GetClaim(Claims.Subject));
            if (user is null)
            {
                return Forbid(
                    new AuthenticationProperties(new Dictionary<string, string?>
