...
using LegendaryTelegram.Server.Models.Manage;
...
public class ManageController : Controller
{
    ...
    {
        _ = await this.userManager.GenerateChangePhoneNumberTokenAsync(await GetCurrentUserAsync(), phoneNumber);
    ...
    public async Task<IActionResult> ManageLogins(ManageMessageId? message = null)
    {
        ...
        var otherLogins = (await this.signInManager.GetExternalAuthenticationSchemesAsync()).Where(auth => userLogins.All(ul => auth.Name != ul.LoginProvider)).ToList();
        this.ViewData["ShowRemoveButton"] = user.PasswordHash is not null || userLogins.Count > 1;
    ...
    private Task<ApplicationUser> GetCurrentUserAsync() => this.userManager.GetUserAsync(this.User);
